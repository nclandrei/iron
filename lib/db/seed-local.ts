...
async function seed() {
  const client = new Client({
    connectionString: 'postgresql://postgres:postgres@localhost:5432/workout',
  });
  ...
  try {
    ...
    // Create indexes
    await client.query(`CREATE INDEX IF NOT EXISTS idx_workout_logs_exercise ON workout_logs(exercise_id, logged_at DESC);`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_workout_logs_workout ON workout_logs(workout_id, logged_at DESC);`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_exercises_workout ON exercises(workout_id, order_index);`);
    ...
    for (const workout of workouts) {
      await client.query(
        'INSERT INTO workouts (name, day_of_week) VALUES ($1, $2) ON CONFLICT (name, day_of_week) DO NOTHING',
    ...
    // Get workout IDs
    const { rows: workoutRows } = await client.query('SELECT id, name FROM workouts ORDER BY id');
    ...
    for (const set of exerciseSets) {
      ...
      for (let i = 0; i < set.exercises.length; i++) {
        ...
        await client.query(
          ...
          [workoutId, i + 1, ex.name, ex.sets, ex.repsMin, ex.repsMax, ex.weight]
        );
      }
    }
